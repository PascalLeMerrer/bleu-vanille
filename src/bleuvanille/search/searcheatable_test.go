...
func TestPopulateIndex(t *testing.T) {
	...
	i := 0
	for i <= 10 {
		name := fmt.Sprintf("Légume%d" , i)

		eatableVar := eatable.Eatable{
			Name:        name,
			Description: "titi",
			Type:        "Ingredient",
		}

		eatable.Save(&eatableVar)
	...
	{
		results, errSearch := SearchForEatable("Legume1",0,9999)
		assert.NoError(t, errSearch, "Error when searching for Legume1")
		assert.True(t, len(results) == 1, "Error when searching for Legume1 : %d results returned instead of 1", len(results))
	}
